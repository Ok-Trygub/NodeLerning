...
const createPost = async (title, content, userId) => {
    ...
    await newPost.save();
    return await User.findByIdAndUpdate(userId, {$push: {posts: newPost._id}}, {new: true}).populate('posts')
}
...
const createComment = async (message, postId, userId) => {
    ...
    return await Post.findByIdAndUpdate(postId, {$push: {comments: newComment._id}}, {new: true}).populate('comments');
}
