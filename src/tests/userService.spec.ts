...
describe("Verify getAllUsers function", () => {
    ...
    it('should return users and count', async () => {
        ...
        (User.findAndCount as jest.Mock).mockResolvedValueOnce([mockUsers, mockCount]);
    ...
    it('should handle errors', async () => {
        ...
        (User.findAndCount as jest.Mock).mockRejectedValueOnce(new Error(errorMessage));
...
describe(' Verify updateUser function', () => {
    ...
    it('should update user with provided fields', async () => {
        const userId = 1;
        const userDto = {name: 'New Name', email: 'new@example.com', password: 'newpassword'};
        ...
        expect(User.update).toHaveBeenCalledWith({id: userId}, {
            email: userDto.email,
            name: userDto.name,
    ...
    it('should throw error if user is not updated', async () => {
        ...
        await expect(updateUser(userId, userDto)).rejects.toThrowError("User hasn't been updated");
    });
});
...
describe('Verify addProduct function', () => {
    ...
    it('should add product to user', async () => {
        ...
        expect(User.findOne).toHaveBeenCalledWith({ where: { id: userId }, relations: ['products'] });
    ...
    it('should throw error if user or product not found', async () => {
        ...
        await expect(addProduct(userId, productId)).rejects.toThrowError('User or product not found');
    });
