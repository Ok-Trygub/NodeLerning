...
export const updateUser = async (id: number, userDto: UpdateUserDto): Promise<User> => {
...
export const addProduct = async (userId: number, productId: number): Promise<User> => {
    const user = await User.findOne({where: {id: userId}, relations: ['products']});
